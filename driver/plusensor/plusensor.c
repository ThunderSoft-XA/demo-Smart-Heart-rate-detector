...
uint8_t getHeartRateValue(uint32_t* heartrate )			
{
	...
	while( i < 60 )
	{
		...
		if(QAPI_OK == status)
		{
			...
			{
				Signal = result.nMicrovolts >> 2;
				gizLog(LOG_INFO,"get ADC value , Signal = %d, result.nCode = %d, result.nMicrovolts = %d\n", Signal, result.nCode, result.nMicrovolts); 
			}
		}
		else
		{
		...
		{  
			if(Signal < T)													// T is the trough	(波谷)
			{                       
		...
		{          
			P = Signal;                             // P is the peak（波峰）,keep track of highest point in pulse wave
			gizLog(LOG_INFO,"Find peak, P = %d\n", P);
		}                                        

		//  NOW IT'S TIME TO LOOK FOR THE HEART BEAT		（找心率）
		// signal surges up in value every time there is a pulse
		if (Num > 250)				// > 0.25ms, 
		{                                   // avoid high frequency noise	（避免高频噪音）
			...
			{        
				...
				*heartrate = BPM;
				gizLog(LOG_INFO,"get heart rate value , heartrate = %d\n", *heartrate); 
		...
		qapi_Timer_Sleep(2, QAPI_TIMER_UNIT_MSEC, true);				//2ms读取一次
		i++;
	}
	/*
    ...
    if(QAPI_OK != status)
    {
		gizLog(LOG_INFO,"qapi_ADC_Close error\n");
		return -1;
    }
	*/
}
